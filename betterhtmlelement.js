class BetterHTMLElement {
    ...
    css(css) {
        for (let [styleAttr, styleVal] of util.enumerate(css))
    ...
    cacheAppend(keyChildObj) {
        for (let [key, child] of util.enumerate(keyChildObj)) {
            this.append(child);
            this[key] = child;
        }
        return this;
    ...
    cacheChildren(keySelectorObj) {
        for (let [key, selector] of util.enumerate(keySelectorObj))
    ...
    on(evTypeFnPairs, options) {
        const that = this;
        for (let [evType, evFn] of util.enumerate(evTypeFnPairs)) {
    ...
    attr(attrValPairs) {
        for (let [attr, val] of util.enumerate(attrValPairs))
